...
public class AngleSurfaceView extends SurfaceView implements SurfaceHolder.Callback
{
	...
	static final int sMaxObjects = 200;
	...
	private void init(Context context)
	{
		...
		//mHolder.setFixedSize(hwWidthPx, hwHeightPx);
	...
	{
		for (AngleObject ao: mChilds){
			if (ao!=null){ao.invalidateHardwareBuffers();}
		}
		mGLThread.onResume();
	}
	...
	public void sizeChanged(GL10 gl, int width, int height)
	{
		...
		gl.glLoadIdentity();
		gl.glOrthof(0.0f, roWidth, roHeight, 0.0f, -10.0f, 10.0f);
...
//		gl.glCullFace(GL10.GL_BACK);
	...
	public void step(float secondsElapsed)
	{
		...
		for (int t=0;t<mChildsCount;t++)
		{
			if (mChilds[t].mDie)
			{
				...
				if (mMotionPause==false){mChilds[t].step(secondsElapsed);}
		}
	}
	...
	public AngleObject addObject(AngleObject object)
	{
		...
		for (int t=0;t<mChildsCount;t++)
		{
			if (mChilds[t]==object)//check to see if already exsists in main childs
				return object;
		}
		for (int t=0;t<mNewChildsCount;t++)
		{
			if (mNewChilds[t]==object)//check to see if already exsists in main New childs
				return object;
		}
		...
		Log.v("earthdefensebeta","done adding angle object");
		return object;
	}
	...
	public void PauseMotion() {
		this.mGLThread.PauseMotion();
		mMotionPause=true;
	}
	public void UnPauseMotion() {
		this.mGLThread.UnPauseMotion();
		mMotionPause=false;
	}

	public void MoveObjectToTop(int fCIdx) {
//		if (mChildsCount-1>=0){
//			AngleObject tmp = mChilds[fCIdx];
//			for (int i = fCIdx;i+1<mChildsCount;i=i+1){
//				mChilds[i]= mChilds[i+1];	
//			}
//			mChilds[mChildsCount-1]=tmp;
//			
//			//mChilds[fCIdx]= tmp;
//		}
	}
	public void MoveObjectToBottom(AngleObject nobj) {
		MoveObjectToBottom(WhatIdx(nobj));	
	}
	
	public void MoveObjectToBottom(int fCIdx) {
		if (mChildsCount-1>=0){return;}
		AngleObject tmp = mChilds[fCIdx];
		
		for (int i=fCIdx;i>0;i--){
			mChilds[i]=mChilds[i-1];
		}
		mChilds[0]= tmp;
	...
	public int WhatIdx(AngleObject nobj){
		for (int i = 0;i<mMaxObjects;i=i+1){
			if(nobj == mChilds[i]){
				return i;
				}	
		}
		for (int i = 0;i<mMaxObjects;i=i+1){
			if(nobj == mNewChilds[i]){
				return i+mChildsCount;
				}	
		}
