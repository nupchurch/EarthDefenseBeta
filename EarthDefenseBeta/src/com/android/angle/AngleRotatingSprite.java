...
public class AngleRotatingSprite extends AngleAbstractSprite
{
	...
	{
		Log.v("way2fungames","start RSInvTexCordBuffIdx");
		...
		Log.v("way2fungames","end RSInvTexCordBuffIdx");
	}
		...
		if (roLayout != null)
		{
			if (roLayout.roTexture != null)
			{
				if (roLayout.roTexture.mHWTextureID > -1)
				{
					...
					gl.glTranslatef(mPosition.getmX(), mPosition.getmY(), 0);
					if (mRotation != 0)
						gl.glRotatef(-mRotation, 0, 0, 1);
					if ((mScale.getmX() != 1) || (mScale.getmY() != 1))
						gl.glScalef(mScale.getmX(), mScale.getmY(), 1);
