...
public class AngleString extends AngleObject
{
	...
	{
		float left=mPosition.getmX();
		if (mAlignment == aRight)
			left=mPosition.getmX() - mWidth;
		else if (mAlignment == aCenter)
			left=mPosition.getmX() - mWidth / 2;

		if (x >= left)
			if (y >= mPosition.getmY() + mFont.mLineat)
				if (x < left + mWidth)
					if (y < mPosition.getmY() + mFont.mLineat + getHeight())
						return true;
		return false;
	}
