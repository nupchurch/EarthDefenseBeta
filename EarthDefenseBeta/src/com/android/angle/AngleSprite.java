		...
		if (roLayout != null)
		{
			if (roLayout.roTexture != null)
			{
				if (roLayout.roTexture.mHWTextureID > -1)
				{
					...
					((GL11Ext) gl).glDrawTexfOES(mPosition.getmX() - (roLayout.getPivot(roFrame).getmX() * mScale.getmX()), AngleSurfaceView.roHeight
							- ((mPosition.getmY() - (roLayout.getPivot(roFrame).getmY() * mScale.getmY())) + (roLayout.roHeight * mScale.getmY())), 0, roLayout.roWidth
							* mScale.getmX(), roLayout.roHeight * mScale.getmY());
				}
				else
